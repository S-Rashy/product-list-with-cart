  ...
  fetch("./data.json")
    ...
    .catch((error) => {
      ...
      menuContainer.innerHTML =
        '<p style="color: rgb(199, 73, 9); text-align: center; padding: 40px;">Error loading menu. Please check if data.json file exists and you are running this on a web server.</p>';
    });
}
  ...
  menuData.forEach((item, index) => {
    menuHTML += `
            <div class="foodCard">
                <img src="${
                  item.image.desktop
                }" class="image" id="image-${index}" alt="${item.name}">
...
confirmBtn.addEventListener("click", function () {
  if (cart.length > 0) {
    alert(
      `Order confirmed! Total: $${totalPrice.toFixed(2)}\n\nItems:\n${cart
